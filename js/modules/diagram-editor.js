...
// ============================================
// ⚡ FASE 6: CROSS-REFERENCES SYSTEM
// ============================================

/**
 * Calculate and generate cross-references between components across pages
 * Based on IEC 750/CEI 3-34 nomenclature system
 */
function calculateCrossReferences(pages) {
  const references = {};
  
  // Scan all pages for component labels
  pages.forEach((page, pageIndex) => {
    (page.elements || []).forEach(component => {
      const baseLabel = getBaseLabel(component.label);
      
      if (!references[baseLabel]) {
        references[baseLabel] = [];
      }
      
      references[baseLabel].push({
        pageIndex: pageIndex + 1, // 1-based page numbering
        pageName: page.name,
        pageType: page.type,
        col: component.col + 1,   // 1-based column numbering
        row: component.row + 1,   // 1-based row numbering
  ...
  // Filter to only multi-page references (cross-references)
  const crossReferences = {};
  Object.entries(references).forEach(([label, locations]) => {
    if (locations.length > 1) {
      crossReferences[label] = locations;
    }
  });
...
/**
 * Get base label without contact suffixes (e.g., KA1.13 -> KA1)
 */
...
/**
 * Format IEC cross-reference (e.g., /2.5 = page 2, column 5)
 */
function formatIECReference(pageIndex, col) {
  return `/${pageIndex}.${col}`;
}

/**
 * Draw cross-references on canvas next to components
 */
function drawCrossReferences(ctx, component, crossReferences, currentPageIndex) {
  const baseLabel = getBaseLabel(component.label);
  const refs = crossReferences[baseLabel];
  
  if (!refs || refs.length <= 1) return;
  
  // Find other pages where this component appears
  const otherPages = refs.filter(ref => ref.pageIndex !== currentPageIndex + 1);
  
  if (otherPages.length === 0) return;
  
  // Position for cross-reference text
  const x = component.col * 80 + 80; // Cell width * cols + margin
  const y = component.row * 60 + 15; // Cell height * rows + offset
  
  // Draw cross-references
  ctx.save();
  ctx.font = '8px Courier New';
  ctx.fillStyle = '#ffaa00'; // Orange color for cross-refs
  
  otherPages.forEach((ref, index) => {
    const refText = formatIECReference(ref.pageIndex, ref.col);
    const offsetY = y + (index * 10);
    
    // Background for visibility
    ctx.fillStyle = 'rgba(0, 0, 0, 0.7)';
    const textWidth = ctx.measureText(refText).width;
    ctx.fillRect(x - 2, offsetY - 8, textWidth + 4, 10);
    
    // Reference text
    ctx.fillStyle = '#ffaa00';
    ctx.fillText(refText, x, offsetY);
  });
...
/**
 * Get detailed IEC nomenclature breakdown for inspector
 */
function getIECNomenclatureBreakdown(label, pageType, machineCode) {
  const nomenclature = new IECNomenclature();
  const parsed = nomenclature.parse(label);
  
  if (!parsed) {
    // Auto-generate nomenclature from component
    const family = nomenclature.getComponentFamily(label);
    return {
      generated: true,
      product: machineCode || 'CRGM',
      location: getLocationFromPageType(pageType),
      identifier: label,
      terminal: null,
      full: nomenclature.generate({
        product: machineCode || 'CRGM',
        location: getLocationFromPageType(pageType),
        identifier: label
      }),
      family: family,
      description: family?.description || 'Componente no identificado'
    };
  }
  ...
  return {
    generated: false,
    ...parsed,
    family: nomenclature.getComponentFamily(parsed.identifier || label),
    description: nomenclature.getComponentFamily(parsed.identifier || label)?.description || 'Componente estándar'
  };
}

/**
 * Get IEC location code from page type
 */
function getLocationFromPageType(pageType) {
  const locationMap = {
    'power': 'Q0',
    'control': 'Q0',
    'protection': 'Q0',
  ...
  return locationMap[pageType] || 'Q0';
}
...
const COMPONENT_RELATIONSHIPS = {
  ...
  // Components that can be both (depending on circuit context)
  hybrid: ['thermal_relay', 'safety_switch', 'safety_module']
};
...
class DiagramEditor {
  constructor(containerEl, diagram) {
    ...
    this.pages = diagram.pages || [{
      ...
      busbars: diagram.busbars || [], // ⚡ FASE 1: Active busbars for this page
      busbarConnections: diagram.busbarConnections || [] // ⚡ FASE 1: Component to busbar connections
    }];
    ...
    // ⚡ FASE 5: ENHANCED INTERACTION STATE
    this.selectedComponents = new Set(); // Multiple selection support
    ...
    // ⚡ FASE 5: MOUSE INTERACTION STATE MACHINE
    this.dragMode = null; // 'move', 'resize', 'select', null
    this.resizeHandle = null; // 'tl', 'tr', 'bl', 'br', 'l', 'r', 't', 'b', null
    this.dragStartPos = null;
    this.dragStartComponents = null; // Snapshot of component positions at drag start
    
    // ⚡ FASE 5: CLIPBOARD SYSTEM
    this.clipboard = [];
    ...
    // ⚡ LEGACY DRAG & DROP STATE (keeping for compatibility)
    this.draggedComponent = null;
    this.dragOffset = { x: 0, y: 0 };

    // ⚡ FASE 4: CABLE MANAGEMENT
    this.cableCounter = diagram.cableCounter || 1; // Global counter for cable numbering
    
  // ⚡ FASE 6: CROSS-REFERENCES SYSTEM
  this.crossReferences = new Map(); // Cached cross-reference data
  this.showCrossReferences = true; // Toggle for cross-reference display
  this.iecNomenclature = new IECNomenclature(); // IEC nomenclature parser
...
class DiagramEditor {
      ...
      <div style="display:flex;flex-direction:column;height:calc(100vh - var(--header-height) - var(--status-bar-height) - 16px);overflow:hidden;">
        ...
        <div class="de-toolbar">
          ...
          <div class="de-toolbar-actions">
            ...
            <button class="de-btn de-btn-delete" data-action="delete" style="display:none;background:var(--color-danger-bg,#330000);color:var(--color-danger,#ff4444);border-color:var(--color-danger,#ff4444);" title="Eliminar componente seleccionado (o click derecho en canvas)">🗑️ Eliminar</button>
  ...
  /**
   * ⚡ FASE 3: Enhanced collision detection for multi-cell components
   * @param {number} col - Column to check
   * @param {number} row - Row to check
   * @returns {Object|null} - Component that occupies this cell, or null
   */
  _getComponentAt(col, row) {
    return this.components.find(c => {
      const compWidth = c.width || 1;
      const compHeight = c.height || 1;
      
      // Check if the cell (col, row) is within the component's bounding box
      return col >= c.col && 
             col < c.col + compWidth && 
             row >= c.row && 
             row < c.row + compHeight;
    }) || null;
  }

  /**
   * ⚡ FASE 3: Check if an area is free for component placement
   * @param {number} col - Starting column
   * @param {number} row - Starting row  
   * @param {number} width - Component width in cells
   * @param {number} height - Component height in cells
   * @param {string} excludeId - Component ID to exclude from check (for resizing)
   * @returns {boolean} - True if area is free
   */
  _isAreaFree(col, row, width = 1, height = 1, excludeId = null) {
    // Check bounds
    if (col < 0 || row < 0 || 
        col + width > this.COLS || 
  ...
  // Generate next label for component type
  _nextLabel(type) {
    const def = IEC_COMPONENTS[type];
    if (!def?.prefix) return `${type}1`;
  ...
  // Enhanced drag state reset with proper cleanup
  _resetDragState() {
    this.dragMode = null;
  ...
  // Get resize handle at specific position
  _getResizeHandleAt(x, y) {
    if (this.selectedComponents.size !== 1) return null;
    
    const compId = Array.from(this.selectedComponents)[0];
    const comp = this.components.find(c => c.id === compId);
    if (!comp || (!comp.width || comp.width === 1) && (!comp.height || comp.height === 1)) return null;
  ...
  // Snapshot selected components for drag operations
  ...
  _refreshColors() {
    ...
    this._colors = {
      bg: root.getPropertyValue('--color-bg').trim() || '#0a0a0a',
      text: root.getPropertyValue('--color-text').trim() || '#e0e0e0',
      grid: root.getPropertyValue('--color-border').trim() || '#333333',
      border: root.getPropertyValue('--color-border').trim() || '#555555',
      success: root.getPropertyValue('--color-success').trim() || '#00ff41',
      danger: root.getPropertyValue('--color-danger').trim() || '#ff4444'
    };
  }

  // ============================================
  // ENHANCED CANVAS EVENTS WITH DRAG & DROP
  // ============================================
  _bindCanvasEvents() {
    ...
    this.canvas.addEventListener('mousedown', (e) => {
      ...
      if (resizeInfo && !this.placingType) {
        ...
        this.dragStartComponents = { ...resizeInfo.component }; // Snapshot
  ...
  // Handle click events (place component or select)
  ...
  undo() {
    if (this.isReadonly || this.historyIndex <= 0) {
      this.statusEl.textContent = '⚠️ No hay nada que deshacer';
      return;
    }

    this.historyIndex--;
    this._restoreFromHistory();
    this.statusEl.textContent = `↩️ Deshecho (${this.history.length - this.historyIndex - 1} acciones disponibles para rehacer)`;
  }

  redo() {
    if (this.isReadonly || this.historyIndex >= this.history.length - 1) {
      this.statusEl.textContent = '⚠️ No hay nada que rehacer';
      return;
    }

    this.historyIndex++;
    this._restoreFromHistory();
    this.statusEl.textContent = `↪️ Rehecho (${this.historyIndex} acciones en historial)`;
  }
  ...
  _restoreFromHistory() {
    ...
    this.counters = JSON.parse(JSON.stringify(snapshot.counters));
  ...
  _updateUndoRedoButtons() {
    ...
    if (undoBtn) {
      undoBtn.disabled = this.isReadonly || this.historyIndex <= 0;
      undoBtn.style.opacity = undoBtn.disabled ? '0.4' : '1';
      undoBtn.style.cursor = undoBtn.disabled ? 'not-allowed' : 'pointer';
    }

    if (redoBtn) {
      redoBtn.disabled = this.isReadonly || this.historyIndex >= this.history.length - 1;
  ...
  // ============================================
  // ⚡ FASE 4: ADVANCED WIRING AND ROUTING
  // ============================================

  /**
   * Generate automatic cable label (-W1, -W2, etc.)
   * @returns {string} - Next available cable label
   */
  _generateCableLabel() {
  ...
  /**
   * Get IEC wire color by name
   * @param {string} colorName - Color name (e.g., 'Negro', 'Azul')
   * @returns {Object} - Wire color info with hex, code, etc.
   */
  _getWireColorByName(colorName) {
    const colorMap = {
      'Negro': IEC_WIRE_COLORS.BK,
      'Marrón': IEC_WIRE_COLORS.BN,
      'Rojo': IEC_WIRE_COLORS.RD,
      'Naranja': IEC_WIRE_COLORS.OG,
      'Amarillo': IEC_WIRE_COLORS.YW,
      'Verde': IEC_WIRE_COLORS.GN,
      'Azul': IEC_WIRE_COLORS.BU,
      'Violeta': IEC_WIRE_COLORS.VT,
      'Gris': IEC_WIRE_COLORS.GY,
      'Blanco': IEC_WIRE_COLORS.WH,
      'Amarillo/Verde': IEC_WIRE_COLORS.GNYE,
      'Rosa': IEC_WIRE_COLORS.PK
    };
    return colorMap[colorName] || IEC_WIRE_COLORS.BK;
  }
   ...
   * @param {Object} fromTerminal - Source terminal info {side}
   * @param {Object} toTerminal - Destination terminal info {side}
   * @returns {Object} - Route with type and waypoints
   */
  _calculateWireRoute(fromPos, toPos, fromTerminal = null, toTerminal = null) {
    const fx = fromPos.x;
    const fy = fromPos.y;
    const tx = toPos.x;
    const ty = toPos.y;
    
    const deltaX = Math.abs(fx - tx);
    const deltaY = Math.abs(fy - ty);
    
    // Straight connections for very close terminals
    if (deltaX < 5) {
      return {
        type: 'vertical',
    ...
    // Orthogonal routing with intelligent waypoint calculation
    const cornerRadius = 8;
    const waypoints = [];
    
    // Determine routing strategy based on terminal sides
    const fromSide = fromTerminal?.side;
    const toSide = toTerminal?.side;
    
    if (fromSide === 'bottom' && toSide === 'top') {
      // Standard vertical cascade (most common)
      const midY = (fy + ty) / 2;
      waypoints.push({ x: fx, y: midY });
      waypoints.push({ x: tx, y: midY });
    } else if (fromSide === 'right' && toSide === 'left') {
      // Horizontal left-to-right
      const midX = (fx + tx) / 2;
      waypoints.push({ x: midX, y: fy });
      waypoints.push({ x: midX, y: ty });
    } else if (fromSide === 'bottom' && toSide === 'left') {
      // Down then left
      waypoints.push({ x: fx, y: ty });
    } else if (fromSide === 'right' && toSide === 'top') {
      // Right then up
      waypoints.push({ x: tx, y: fy });
    } else {
    ...
    return {
      type: 'orthogonal',
      points: [{ x: fx, y: fy }, ...waypoints, { x: tx, y: ty }],
      cornerRadius: cornerRadius
    };
  }

  /**
   * Draw a wire route with rounded corners and labels
   * @param {CanvasRenderingContext2D} ctx - Canvas context
   * @param {Object} route - Route object with points and cornerRadius
   * @param {Object} wire - Wire object with properties
   * @param {string} wireColor - Wire color hex
   */
  _drawWireRoute(ctx, route, wire, wireColor) {
    if (!route.points || route.points.length < 2) return;
    
    const points = route.points;
    const cornerRadius = route.cornerRadius || 0;
    ...
    if (cornerRadius > 0 && points.length > 2) {
      ...
      for (let i = 1; i < points.length - 1; i++) {
        ...
        // Calculate vectors
        const vec1X = curr.x - prev.x;
        const vec1Y = curr.y - prev.y;
        const vec2X = next.x - curr.x;
        const vec2Y = next.y - curr.y;
        ...
        if (len1 > 0 && len2 > 0) {
          const norm1X = vec1X / len1;
          const norm1Y = vec1Y / len1;
          const norm2X = vec2X / len2;
          const norm2Y = vec2Y / len2;
          
          // Calculate corner points
          const radius = Math.min(cornerRadius, len1 / 2, len2 / 2);
          const corner1X = curr.x - norm1X * radius;
          const corner1Y = curr.y - norm1Y * radius;
          const corner2X = curr.x + norm2X * radius;
          const corner2Y = curr.y + norm2Y * radius;
          
          // Draw line to corner, then arc to next segment
          ctx.lineTo(corner1X, corner1Y);
          ctx.arcTo(curr.x, curr.y, corner2X, corner2Y, radius);
        } else {
  ...
  /**
   * Draw cable label with background at the midpoint of a route
   * @param {CanvasRenderingContext2D} ctx - Canvas context
   * @param {Object} route - Route object with points
   * @param {Object} wire - Wire object with label and properties
   * @param {string} wireColor - Wire color hex
   */
  _drawCableLabel(ctx, route, wire, wireColor) {
    if (!wire.cableLabel && !wire.section) return;
    if (!route.points || route.points.length < 2) return;
    ...
    // If we have an even number of points, interpolate between middle points
    let midX, midY;
    if (totalPoints % 2 === 0 && midIndex > 0) {
      const point1 = route.points[midIndex - 1];
      const point2 = route.points[midIndex];
    ...
    // Prepare label text
    const primaryLabel = wire.cableLabel || `-W${this.wires.indexOf(wire) + 1}`;
    const secondaryLabel = `${wire.section || '1.5'}mm² ${this._getWireColorByName(wire.color || 'Negro').code}`;
    ...
    const labelWidth = maxWidth + 8; // 4px padding each side
    ...
    // Draw primary label (cable ID)
    ctx.fillStyle = '#ffffff';
    ctx.font = 'bold 8px Courier New';
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    ctx.fillText(primaryLabel, midX, midY - 3);
    
    // Draw secondary label (section and color)
    ctx.fillStyle = 'rgba(255, 255, 255, 0.8)';
    ctx.font = '7px Courier New';
    ctx.fillText(secondaryLabel, midX, midY + 5);
  ...
  // ============================================
  // ⚡ FASE 2: TERMINAL POSITIONING HELPERS
  // ============================================
  
  /**
   * Get the exact (x, y) coordinates of a specific terminal on a component
   * @param {Object} component - Component object
   * @param {string} terminalId - Terminal ID from the schema (e.g., 'L1', 'A1', 'I0')
   * @returns {Object|null} - {x, y, terminal} or null if not found
   */
  _getTerminalPosition(component, terminalId) {
    if (!component) return null;
    
    const schema = COMPONENT_TERMINAL_SCHEMA[component.type];
    if (!schema || !schema.terminals) {
      // Fallback to legacy behavior (top/bottom center)
      const x = component.col * this.CELL_W + this.CELL_W / 2;
      if (terminalId === 'top') {
        return { x, y: component.row * this.CELL_H + 8, terminal: { label: 'TOP', type: 'generic' } };
      } else if (terminalId === 'bottom') {
        return { x, y: component.row * this.CELL_H + this.CELL_H - 8, terminal: { label: 'BOT', type: 'generic' } };
      }
      return null;
    }
    ...
    const baseX = component.col * this.CELL_W;
    const baseY = component.row * this.CELL_H;
    const w = this.CELL_W;
    const h = this.CELL_H;
    ...
    switch (terminal.side) {
      case 'top':
        x = baseX + w/2 + (w * terminal.offset);
        y = baseY + 8; // Fixed distance from top edge
        break;
      case 'bottom':
        x = baseX + w/2 + (w * terminal.offset);
        y = baseY + h - 8; // Fixed distance from bottom edge
        break;
      case 'left':
        x = baseX + 8; // Fixed distance from left edge
        y = baseY + h/2 + (h * terminal.offset);
        break;
      case 'right':
        x = baseX + w - 8; // Fixed distance from right edge
        y = baseY + h/2 + (h * terminal.offset);
        break;
      default:
        return null;
  ...
  /**
   * Get all available terminals for a component
   * @param {Object} component - Component object
   * @returns {Array} - Array of terminal objects with id, label, type, side, position
   */
  _getComponentTerminals(component) {
    if (!component) return [];
    
    const schema = COMPONENT_TERMINAL_SCHEMA[component.type];
    if (!schema || !schema.terminals) {
      // Fallback to legacy terminals
      const def = IEC_COMPONENTS[component.type];
      if (!def || !def.terminals) return [];
      
      const terminals = [];
      if (def.terminals.top) terminals.push({ id: 'top', label: 'TOP', type: 'generic', side: 'top' });
      if (def.terminals.bottom) terminals.push({ id: 'bottom', label: 'BOT', type: 'generic', side: 'bottom' });
      return terminals;
    }
    
    return Object.entries(schema.terminals).map(([id, terminal]) => ({
      id,
...
class IECNomenclature {
  ...
  validate(nomenclature) {
    ...
    if (parsed.location && !this.LOCATION_CODES[parsed.location]) {
      ...
      // ⚡ FASE 5: Enhanced selection with Ctrl+Click for multiple selection
      const comp = this._getComponentAt(col, row);
      if (comp) {
        if (e.ctrlKey || e.metaKey) {
          // Ctrl+Click: Toggle component in selection
